...
# Docker Stack Manager Installer
# This script installs the Docker Stack Manager

echo "Installing Docker Stack Manager..."

# Define paths
SCRIPT_PATH="$HOME/bin/ds"
...
# Create aliases
cat > "$HOME/.docker_stack_aliases" << EOF
# Docker Stack Manager Aliases
alias ds="$SCRIPT_PATH"
alias dsl="$SCRIPT_PATH list"
alias dsu="$SCRIPT_PATH up"
alias dsd="$SCRIPT_PATH down"
alias dsr="$SCRIPT_PATH restart"
alias dsp="$SCRIPT_PATH ps"
alias dslogs="$SCRIPT_PATH logs"
alias dsupdate="$SCRIPT_PATH update"
alias dsb="$SCRIPT_PATH backup"
EOF

# Add source to shell config if not already there
for config_file in ~/.bashrc ~/.zshrc; do
    if [ -f "$config_file" ]; then
        if ! grep -q "source ~/.docker_stack_aliases" "$config_file"; then
            echo "" >> "$config_file"
            echo "# Docker Stack Manager" >> "$config_file"
            echo "source ~/.docker_stack_aliases" >> "$config_file"
        fi
    fi
done
...
echo ""
echo "Docker Stack Manager installed successfully!"
echo "The 'ds' command is now available. Run 'ds help' for usage information."
echo ""
echo "To use immediately in this shell, run:"
echo "  source ~/.docker_stack_aliases"
echo ""
echo "Directory structure created at $DOCKER_BASE_DIR"
echo "You can now create your first stack with: ds create mystack"