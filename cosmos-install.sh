...
setup_cosmos() {
    if [ "$BETA" -eq 1 ]; then
        LATEST_RELEASE=$(curl -s https://api.github.com/repos/azukaar/Cosmos-Server/releases | grep "tag_name" | cut -d '"' -f 4 | head -n 1)
    else
    ...
    if [ "$DRY_RUN" -eq 1 ]; then
        ...
        print_dry "Download and verify Cosmos $LATEST_RELEASE release zip and extract to /opt/cosmos"
        return
    fi
    ...
    print_status "Downloading Cosmos release $LATEST_RELEASE..."
