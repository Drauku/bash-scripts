#!/bin/bash
################################################################################
# Place this file in $HOME/docker/ and name it docker_functions
# A quick and easy way to do this is to run one of these download commands:
# wget 'https://raw.githubusercontent.com/QNAP-HomeLAB/docker-scripts/master/docker_functions' -O $HOME/docker/docker_functions
# curl 'https://raw.githubusercontent.com/QNAP-HomeLAB/docker-scripts/master/docker_functions' > $HOME/docker/docker_functions
# git archive --remote=https://github.com/QNAP-HomeLAB/docker-scripts.git HEAD:docker_functions $HOME/docker/docker_functions | tar -xvf -
################################################################################
...
####################### docker uid/gid/directories setup #######################
    ...
    export docker_shared="$docker_dir/shared"
    ...
    ## configuration types
    export docker_scopes=("build" "local" "swarm")

    export docker_appdata=("${local_appdata}" "${swarm_appdata}")
    export docker_configs=("${local_configs}" "${swarm_configs}")

    export docker_fnc_file="$docker_shared/docker_functions"

    export docker_env_example="$docker_shared/docker.env.example"
    export docker_env_file="$docker_secrets/.docker.env"
    ...
    docker_uid=$(id -u "${docker_usr:-docker}"); export docker_uid
    # echo "DEBUG: docker UID: ${docker_uid}"
    docker_gid=$(id -g "${docker_usr:-docker}"); export docker_gid
    # echo "DEBUG: docker GID: ${docker_gid}"
    ...
    if [[ $(id -u) -ne 0 ]] && [[ -e $(command -v sudo) ]]; then var_sudo="sudo"; else var_sudo=""; fi; #unset var_sudo; fi;
...
########################## docker functions ############################
    ...
    fnc_extract_option(){ # USAGE: fnc_extract_option $@
        ...
        if [[ ${#opts[@]} -gt 1 ]]; then
            ...
            operands=("${args[@]}"); #echo "DEBUG: operands: ${operands[*]}";
            option=("${opts[@]}") ; #echo "DEBUG: option: ${option[*]}";
        fi
        }
# fnc_extract_option(){
    #     local opt
    ...
    fnc_file_download(){ # USAGE: fnc_file_download <url> <filepath> [symlink]
        # echo -e "\n DEBUG: fnc_file_download args: $*"
        fnc_extract_option "$@";
        local file_url="${operands[0]}"; #echo "DEBUG: file_url: $file_url";
        local filename="${operands[1]}"; #echo "DEBUG: filename: $filename";
        local filelink="${operands[2]}"; #echo "DEBUG: filelink: $filelink";
        # local file_url="${1}";
        # local filename="${2}";
        # local filelink="${3}";
...
fnc_find_and_move_file(){ # USAGE: fnc_find_and_move_file <filename> <target_dir> [search_dir]
    ...
    ## usage: fnc_symlink_create <symlink> <target>
    fnc_symlink_create(){ if [[ ! -f "$1" ]]; then ln -s "$2" "$1"; fi; }

    fnc_initial_setup(){
        ## create docker data and config directories if they do not exist
        fnc_dir_create "${docker_dir}" "${perms_data}"
        ## if necessary, create $HOME/docker symlink and update $docker_dir
        if [[ "${docker_dir}" != "$HOME/docker" ]]; then
            fnc_symlink_create "${HOME}/docker" "${docker_dir}";
            docker_dir="${HOME}/docker";
        fi
        ## create docker data and config directories if they do not exist
        fnc_dir_create "${docker_secrets}" "${perms_data}"
        fnc_dir_create "${docker_shared}" "${perms_data}"
        ...
        ## move docker_functions to $HOME/docker/shared
        fnc_find_and_move_file "docker_functions" "${docker_shared}"

        ## download .bash_env if it does not exist
        if [[ ! -f "$HOME/.bash_env" ]]; then curl -s 'https://raw.githubusercontent.com/QNAP-HomeLAB/docker-scripts/master/.bash_env' > "$HOME/.bash_env"; fi;
...
## add $HOME/.docker_env to $HOME/.profile if it does not exist
        if ! grep -q "$docker_env_file" "$HOME/.profile"; then echo "src=$docker_env_file; if [ -f \"\${src}\" ]; then . \"\${src}\"; fi" >> "$HOME/.profile"; fi;

        ## create the $HOME/.docker_fnc symlink if it does not exist
        fnc_symlink_create "$HOME/.docker_fnc" "$docker_fnc_file";
        ## add $HOME/.docker_fnc to $HOME/.profile if it does not exist
        if ! grep -q "$docker_fnc_file" "$HOME/.profile"; then echo "src=$docker_fnc_file; if [ -f \"\${src}\" ]; then . \"\${src}\"; fi" >> "$HOME/.profile"; fi;
    ...
    ## initial setup tasks
    if [[ ! -f "$docker_fnc_file" ]]; then fnc_initial_setup; fi
    ...
    set_scope_vars(){
        docker_scope="${operands[0]}"; unset "operands[0]";
        # echo "DEBUG: 'set_scope_vars' --docker_scope: ${docker_scope}"
        # if test -z "${docker_scope}"; then local docker_scope="${operands[0]}"; fi
        # echo "DEBUG: 'set_scope_vars' --docker_scope: ${docker_scope} --operands: ${operands[*]}"
        case "${docker_scope}" in
            ("build")
            ...
            ("swarm")
                ...
                ;;
            (*) msg_error "INVALID CONFIG TYPE: $1 OR $docker_scope" "Please inform the script maintainer."; return;;
        esac
        }
    ...
    fnc_check_dir(){ if test ! -d "$1"; then msg_warning "Docker container directory does not exist." "Use \`dcf $1\` / \`dwf $1\` to create."; return; fi; }
    ...
    fnc_configs_list(){ ## USAGE: fnc_configs_list <docker_scope>
        fnc_extract_option "$@";
        local docker_scope="${operands[0]}"; unset "operands[0]";
        set_scope_vars "$docker_scope";
        ...
        ## print config list
        if [[ "${#config_list[@]}" -eq 0 ]];
        then msg_warning "No $docker_scope configs found." "Use \`dcf/dwf $<appname>\` to create one."; return;
        else echo -e " > DOCKER CONFIG LIST FOR \`$docker_scope\` CONTAINERS <\n ${config_list[*]}";
        fi
        }

    docker_compose_config(){
        fnc_extract_option "$@"
        local docker_scope="${operands[0]}";
        set_scope_vars "$docker_scope"
        docker compose -f "${configs_path}/$1/${compose_file}" config;
        }
    ...
    docker_compose_logs(){
        ...
        local docker_scope="${operands[0]}";
        set_scope_vars "$docker_scope";
        # (cd "${configs_path}/$1" && docker compose logs -f)
        (cd "${configs_path}/$1" && docker logs -tf --tail="50" "${operands[1]}")
        }
...
# #################### general docker functions ####################
    ...
    alias dknl="docker_list_networks"

    ## docker volume list function
    docker_volumes_list(){ docker volumes ls; }
    alias dkvl="docker_volumes_list"

    ## docker image list function
    docker_images_list(){ docker images; }
    alias dki="docker_images_list"
    ...
    # dklocal_appdata(){ cd "${local_appdata}/$1" 2>/dev/null || echo; return; }
    dklocal_appdata(){ cd "${local_appdata}/$1" || return; }
    alias dkla="dklocal_appdata"
    dklocal_configs(){ cd "${local_configs}/$1" 2>/dev/null || return; }
    alias dkl="dklocal_configs" #"compose"
    ...
    dkswarm_appdata(){ cd "${swarm_appdata}/$1" || return; }
    alias dkwa="dkswarm_appdata"
    dkswarm_configs(){ cd "${swarm_configs}/$1" || echo; return; }
    alias dkw="dkswarm_configs"
    ...
    #     }
    # vpncheck(){ dl_cmd_check; echo "      Host IP: $("${dl_cmd}" ifconfig.me)" && echo " Container IP: $(docker container exec -it "${*}" "${dl_cmd}" ipinfo.io\ip)"; }
    ...
    # ctipcheck(){ echo " Container IP: $(docker container exec -it "${*}" wget -qO- ipinfo.io)"; }
...
########################## docker permissions update ###########################
    ...
    #             case "${option[0]}" in
    #                 ("-a"|"--all") files_dir="${configs_path:?}" ;;
    #                 (*) msg_error "Invalid option: ${option[0]}"; return 1;;
    #             esac
    #             ;;
    #         *) files_dir="${configs_path:?}/${1}" ;;
    #     esac
    ...
    docker_folder_permissions(){ # usage: docker_folder_permissions [option] <file list>
        echo "DEBUG: docker_folder_permissions --args: '${*}'"
        fnc_extract_option "$@";
        echo "DEBUG: docker_folder_permissions --args: '${*}' --option: '${option[*]}' --operands: '${operands[*]}'"
                fnc_check_app "${operands[@]}";
        case "${option[0]}" in
                "-a"|"--all")
                    ...
                    ;;
                *) msg_failure "INVALID OPTION" "Only the '-a' or '--all' option is supported.";;
        esac
        ## update each existing folder in the provided operands list
        for stack in "${operands[@]}"; do
            # update specified docker folder permissions
                    ...
                    if [[ -d "${appconf_dir:?}" ]]
            then ## appdata folder and file permissions
                    ${var_sudo}chown -R "${docker_uid}:${docker_gid}" "${appdata_dir:?}";
                    ${var_sudo}chmod -R "${perms_data}" "${appdata_dir:?}";
            else msg_failure "DIRECTORY DOES NOT EXIST" "Unable to update \`${appconf_dir:?}\` permissions";
                    fi;
            if [[ -d "${appdata_dir:?}" ]];
            then ## local folder and file permissions
                    ${var_sudo}chown -R "${docker_uid}:${docker_gid}" "${appconf_dir:?}";
                    ${var_sudo}chmod -R "${perms_conf}" "${appconf_dir:?}";
                    else msg_failure "DIRECTORY DOES NOT EXIST" "Unable to update \`${appdata_dir:?}\` permissions";
            fi;
    ...
    #     done
    #     }
    alias dkp="docker_folder_permissions --all"

########################## docker_files and docker_folders ############################

    fnc_env_create(){
        fnc_extract_option "${@}";
        echo "DEBUG: 'fnc_env_create' --option: ${option[*]} --operands: ${operands[*]}"
        set_scope_vars "${operands[0]}";
        ...
        # case "${1}" in
            ...
            *) # symlink .env to .docker.env.example if it does not exist
                fnc_symlink_create "${configs_path:?}/$1/.env" "${docker_env_file}";
    ...
    docker_folders_create(){
        # echo "DEBUG: docker_folders_create args: ${*} option: '${option[*]}' operands: '${operands[*]}'"
        # echo "DEBUG: docker_folders_create configs_path: '${configs_path}' appdata_path: '${appdata_path}'"
        # fnc_extract_option "$@" ## done in docker_folder_actions
        # docker_scope="${operands[0]}"; unset "operands[0]"; ## done in docker_folder_actions
        # set_scope_vars "${docker_scope:?}" ## done in docker_folder_actions
        for stack in "${operands[@]}"; do
            docheck=0;
            fnc_check_app "$stack";
            ...
            case "$docheck" in
                "1") msg_failure "Directory \`${appdata_path:?}/${stack}\` already exists" "Check name or delete before recreating.";;
                "2") msg_failure "Directory for \`${configs_path:?}/${stack}\` already exists" "Check name or delete before recreating.";;
                "3") msg_failure "Docker appdata and configs directories for the \`${stack}\` application already exist." "Check name or delete before recreating.";;
            esac
        done
        echo
        }
    ...
    docker_folder_actions(){
        echo "DEBUG: docker_folder_actions --args: '${*}'"
        fnc_extract_option "$@"
        echo "DEBUG: docker_folder_actions --args: ${*} --option: '${option[*]}' --operands: '${operands[*]}'"
        set_scope_vars "${operands[0]}";
        echo "DEBUG: docker_folder_actions --appdata_path: '${appdata_path}' --configs_path: '${configs_path}'"
        # docker_scope="${operands[0]}"; unset "operands[0]";
        operands=("${operands[@]:1}"); ## remove config type from operands
        if test -n "${option[0]}"; then
                case "${option[0]}" in
                    "-c"|"--create") docker_folders_create "${operands[@]}" ;;
                    "-d"|"--delete") docker_folders_delete "${operands[@]}" ;;
                    "-r"|"--remove") docker_folders_delete "${operands[@]}" ;;
                    *) msg_failure "INVALID OPTION '${option[*]}' USED" "Only '-c' or '-d' or '-r' options are allowed";
                esac
                else docker_folders_create "${operands[@]}"
        fi;
        }
    ...
    docker_networks_create(){
        ...
        docker network create --opt "encrypted" --scope "$1" --driver "$2" --subnet "${net_prefix_internal_only}.0/24" --gateway "${net_prefix_internal_only}.254" --attachable "internal_only"
...
alias dknc="docker_networks_create"
...
########################### docker local functions ###########################
    ...
    docker_local_start(){
        echo " DEBUG: 'docker_local_start' --args: '${*}'"
        applist=( "$@" )
        echo " DEBUG: 'docker_local_start' --applist: '${applist[*]}'"
        docker_scope="local";
        set_scope_vars "${docker_scope}";
        echo " DEBUG: 'docker_local_start' --docker_scope: '${docker_scope}' --configs_path: '${configs_path}'"
        fnc_compose_start(){
            for app in "${applist[@]}"; do
                if [[ -f "${local_configs}/$app/${local_compose}" ]]; then
                    fnc_env_create "$docker_scope" "$app";
    ...
    docker_local_stop(){
        ...
        # docker stop "$1" && docker container rm "$1" # need to script the search for services and stop each by name
        fi
        echo
        }
...
alias dwn="docker_networks_create swarm overlay"
    ...
    alias dknodes="docker_list_swarm_nodes"
